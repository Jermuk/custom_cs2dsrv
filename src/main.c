	...
	while (1)
	{
		if (mtime() - mstime >= fps) //Hope it does an constant fps
		{
			...
			if (FD_ISSET(readsocket, &descriptor))
			{
				...
				{
					int id = IsPlayerKnown(newclient.sin_addr,
							newclient.sin_port); /// Function returns id or -1 if unknown
					if (id != -1)///When the player is known execute other commands as when the player is unknown
					{
						if (ValidatePaket(buffer, id)) ///Checks and raise the packet numbering if necessary
						{
							...
							while (control)
							{
								...
								{
								case 1:
									rtn = confirmation_known(message, tempsize,
											id, readsocket);
									break;
								case 3:
									rtn = connection_setup_known(message,
											tempsize, newclient.sin_addr,
											newclient.sin_port, id);
									break;
								case 7:
									rtn = fire(message, tempsize, id,
											readsocket);
									break;
								case 8:
								...
								case 12:
									...
									break;
								case 13:
									rtn = posrotupdaterun(message, tempsize,
											id, readsocket);
									break;
								case 14:
									rtn = posrotupdatewalk(message, tempsize,
											id, readsocket);
									break;
								case 16:
									rtn = reload(message, tempsize, id,
											readsocket);
									break;
								case 20:
								...
								case 23:
									rtn
											= buy(message, tempsize, id,
													readsocket);
									break;
								case 28:
									// Spray 28 - 0 - x x - y y - color
									rtn = spray(message, tempsize, id,
											readsocket);
									break;
								case 32:
								...
								case 249:
									...
									break;
								case 252:
									rtn = joinroutine_known(message, tempsize,
											id, readsocket);
									break;
								case 253:
