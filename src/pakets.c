...
int posrotupdatewalk(unsigned char *message, int length, int id, int writesocket)
{
    ...
    if(rotation >= -180 && rotation <= 180)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            SendPosRotUpdate(id, player[id].x, player[id].y, 0, player[id].rotation, writesocket);
            break;
        default:
            break;
        }
...
int posrotupdaterun(unsigned char *message, int length, int id, int writesocket)
{
    ...
    if(rotation >= -180 && rotation <= 180)
    {
        ...
        case 0:
            ...
            break;
        case 1:
            SendPosRotUpdate(id, player[id].x, player[id].y, 1, player[id].rotation, writesocket);
            break;
        default:
            break;
        }
...
int ping_serverlist(unsigned char *message, int length, struct sockaddr_in *client, int writesocket)
{
    ...
    if(tempbuffer == NULL) error_exit("Memory error ( ping_serverlist() )");
...
int serverinfo_request(unsigned char *message, int length, struct sockaddr_in *client, int writesocket)
{
    int paketlength = length;
    if(length < 2)
    {
    ...
    */
    ...
    if(buffer == NULL) error_exit("Memory error ( serverinfo_request() )");
...
int joinroutine_unknown(unsigned char *message, int length, struct sockaddr_in *client, int writesocket)
{
...
int specpos(unsigned char *message, int length, int id, int writesocket)
{
    ...
    x = (message[4] << 24) | (message[3]  << 16) | (message[2] << 8) | message[1];
    y = (message[8] << 24) | (message[7]  << 16) | (message[6] << 8) | message[5];
...
int chatmessage(unsigned char *message, int length, int id, int writesocket)
{
    ...
    if (string == NULL) error_exit("Memory error ( joinroutine_known() )\n");
...
int joinroutine_known(unsigned char *message, int length, int id, int writesocket)
{
    ...
    case 1:
    {
        if(player[id].joinstatus == 1)
        {
            ...
            if (player[id].name == NULL) error_exit("Memory error ( joinroutine_known() )\n");
            ...
            if (password == NULL) error_exit("Memory error ( joinroutine_known() )\n");
            ...
            */
            ...
            if (encryption1 == NULL) error_exit("Memory error ( joinroutine_known() )\n");
...
*/
            ...
            if (player[id].spraylogo == NULL) error_exit("Memory error ( joinroutine_known() )\n");
            ...
            if (maphash == NULL) error_exit("Memory error ( joinroutine_known() )\n");
            ...
            if (player[id].win == NULL) error_exit("Memory error ( joinroutine_known() )\n");
            ...
            */
            ...
            if(stringsize != 0) SendToPlayer(buffer, stringsize, id, 1, writesocket);
    ...
    case 3:
    {
        if(player[id].joinstatus == 2)
        {
            ...
            if (pre_authcode_respond == NULL) error_exit("Memory error ( joinroutine_known() )\n");
    ...
    case 5:
    {
        if(player[id].joinstatus == 3)
        {
            ...
            if (mapname == NULL) error_exit("Memory error ( joinroutine_known() )\n");
            ...
            for (i = 1; i <= sv_maxplayers; i++)
            {
                if(player[i].used == 1 && player[i].joinstatus >= 1 && i != id)
                {
                    ...
                    */
                    ...
                    buffer[position] = 0; //Unknown
                    position++;
                    buffer[position] = player[i].slot[player[i].actualweapon].id; //Aktuelle Waffe
                    position++;
                    buffer[position] = 0; //Unknown
                    position++;
                    /*
